    ...
    public function anyData()
    {
        ...
        return Datatables::of($users)
            ...
            ->addColumn('edit', function ($user) {
                return '<a href="' . route("users.edit", $user->id) . '" class="btn btn-success">Редагувати</a>';
            })
            ->add_column('delete', function ($user) { 
                return '<button type="button" class="btn btn-danger delete_client" data-client_id="' . $user->id . '" onClick="openModal(' . $user->id. ')" id="myBtn">Видалити</button>';
            })->make(true);
    }
    ...
    public function taskData($id)
    {
        ...
        return Datatables::of($tasks)
            ...
            ->editColumn('status', function ($tasks) {
                return $tasks->status == 1 ? '<span class="label label-success">Відкрита</span>' : '<span class="label label-danger">Закрита</span>';
            })
    ...
    public function leadData($id)
    {
        ...
        return Datatables::of($leads)
            ...
            ->editColumn('status', function ($leads) {
                return $leads->status == 1 ? '<span class="label label-success">Відкритий</span>' : '<span class="label label-danger">Закритий</span>';
            })
            ->editColumn('client_id', function ($tasks) {
                return $tasks->client->name;
            })
