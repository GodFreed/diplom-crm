...
class TaskActionNotification extends Notification
{
    ...
    public function toArray($notifiable)
    {
        switch ($this->action) {
            case 'created':
                $text = __('":title" був створений користувачем ":creator", і призначений вам', [
                    'title' =>  $this->task->title,
                    'creator' => $this->task->creator->name,
                    ]);
                break;
            case 'updated_status':
                $text = __('":title" був виконаний користувачем ":username"', [
                    'title' =>  $this->task->title,
                    'username' =>  Auth()->user()->name,
                    ]);
                break;
            case 'updated_time':
                $text = __('":username" ввів новий час для ":title"', [
                    'title' =>  $this->task->title,
                    'username' =>  Auth()->user()->name,
                    ]);
                break;
            case 'updated_assign':
                $text = __(':username призначив задачу ":title" для вас', [
                    'title' =>  $this->task->title,
                    'username' =>  Auth()->user()->name,
                    ]);
                break;
